...
class NewDualCrossModalAttention(nn.Module):
    """ Dual CMA attention Layer"""

    def __init__(self, xdim, ydim, activation=None, xsize=16, ysize=16, ratio=8, ret_att=False):
        super(NewDualCrossModalAttention, self).__init__()
        self.activation = activation
        self.ret_att = ret_att
        ...
        # query conv
        self.key_conv1 = nn.Conv2d(
            in_channels=xdim, out_channels=out_channels, kernel_size=1)
        self.key_conv2 = nn.Conv2d(
            in_channels=ydim, out_channels=out_channels, kernel_size=1)
        self.key_conv_share = nn.Conv2d(
            in_channels=out_channels, out_channels=out_channels, kernel_size=1)

        self.linear1 = nn.Linear(ysize*ysize, ysize*ysize)
        self.linear2 = nn.Linear(xsize*xsize, xsize*xsize)

         # separated value conv
        self.value_conv1 = nn.Conv2d(
            in_channels=xdim, out_channels=ydim, kernel_size=1)
        self.gamma1 = nn.Parameter(torch.zeros(1))

        self.value_conv2 = nn.Conv2d(
            in_channels=ydim, out_channels=xdim, kernel_size=1)
        self.gamma2 = nn.Parameter(torch.zeros(1))
    ...
    def forward(self, x, y):
        ...
        def _get_att(a, b):
            proj_key1 = self.key_conv_share(self.key_conv1(a)).view(
                B2, -1, H1*W1).permute(0, 2, 1)  # B, HW, C
            proj_key2 = self.key_conv_share(self.key_conv2(b)).view(
                B2, -1, H2*W2)  # B X C x (*W*H)
            energy = torch.bmm(proj_key1, proj_key2)  # B, HW, HW
        ...
        proj_value_y_on_x = self.value_conv2(y).view(
            B2, -1, H2*W2)  # B, C, HW
        out_y_on_x = torch.bmm(proj_value_y_on_x, att_y_on_x.permute(0, 2, 1))
        out_y_on_x = out_y_on_x.view(B1, C1, H1, W1)
        out_x = self.gamma1*out_y_on_x + x

        proj_value_x_on_y = self.value_conv1(x).view(
            B1, -1, H1*W1)  # B , C , HW
        out_x_on_y = torch.bmm(proj_value_x_on_y, att_x_on_y.permute(0, 2, 1))
        out_x_on_y = out_x_on_y.view(B2, C2, H2, W2)
        out_y = self.gamma2*out_x_on_y + y
...
if __name__ == "__main__":
    x = torch.rand(10, 512, 19, 19)
    y = torch.rand(10, 728, 7, 7)
    dcma = NewDualCrossModalAttention(xdim=512, ydim=728, xsize=19, ysize=7)
